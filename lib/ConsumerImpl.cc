...
void ConsumerImpl::messageReceived(const ClientConnectionPtr& cnx, const proto::CommandMessage& msg,
    ...
    if (metadata.has_num_messages_in_batch()) {
        ...
        LOG_INFO("Receive msg redeliveryCount: " << redeliveryCount << " msg: " << m.getMessageId());
...
void ConsumerImpl::executeNotifyCallback(Message& msg) {
    LOG_INFO("exece msg" + msg.getDataAsString() + " " << msg.getRedeliveryCount());
...
uint32_t ConsumerImpl::receiveIndividualMessagesFromBatch(const ClientConnectionPtr& cnx,
    ...
    for (int i = 0; i < batchSize; i++) {
        ...
        LOG_INFO("batch receive: " << msg.getMessageId() << " originMsgId" << batchedMessage.getMessageId()
                                   << " redeliverCount: " << redeliveryCount);
        if (redeliveryCount >= deadLetterPolicy_.getMaxRedeliverCount()) {
            LOG_INFO("add to batch dlq queue: " << msg.getMessageId());
        ...
        if (cnx->getServerProtocolVersion() >= proto::v2) {
            ...
            for (const auto& msgId : messageIds) {
                processPossibleToDLQ(msgId,
                                     [this, needRedeliverMsgs, &msgId, needCallBack](bool processSuccess) {
                                         if (!processSuccess) {
                                             needRedeliverMsgs->emplace(msgId);
                                         }
                                         if (--(*needCallBack) == 0 && !needRedeliverMsgs->empty()) {
                                             redeliverMessages(*needRedeliverMsgs);
                                         }
                                     });
            }
        }
...
void ConsumerImpl::redeliverMessages(const std::set<MessageId>& messageIds) {
    ...
    if (cnx) {
        if (cnx->getServerProtocolVersion() >= proto::v2) {
            ...
            // todo debug log
            LOG_INFO("Sending RedeliverUnacknowledgedMessages command for Consumer - " << getConsumerId());
        }
    } else {
        LOG_WARN("Connection not ready for Consumer - " << getConsumerId());
    }
        ...
        deadLetterProducer_->getFuture().addListener([self, message, cb](Result res, Producer producer) {
            ...
            producer.sendAsync(msgBuilder.build(), [self, originMessageId, cb](Result res,
                ...
                if (res == ResultOk) {
                    ...
                    self->acknowledgeAsync(originMessageId, [self, originMessageId, cb](Result result) {
                        if (result != ResultOk) {
