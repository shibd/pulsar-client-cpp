...
TEST(ConsumerTest, testPatternSubAllTopic) {
    Client client(lookupUrl);
    auto topicName = "testPatternSubNonPartitionedTopic" + std::to_string(time(nullptr));
    ...
    // verify sub persistent and non-persistent topic
    {
        ...
        ASSERT_TRUE(multiConsumerImplPtr->consumers_.find(topicName1) != boost::none);
        ASSERT_TRUE(multiConsumerImplPtr->consumers_.find(topicName2) != boost::none);
        ASSERT_TRUE(multiConsumerImplPtr->consumers_.find(topicName3) != boost::none);
        ASSERT_TRUE(multiConsumerImplPtr->consumers_.find(topicName4) == boost::none);
